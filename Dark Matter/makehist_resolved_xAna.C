...
#define makehist_resolved_xAna_cxx
#include <TH2.h>
#include <TStyle.h>
#include <TCanvas.h>
    ...
    using namespace std;
    void makehist_resolved_xAna(std::string inputFile){
      ...
      Long64_t nPass[20]={0};
        ...
        TH1F *nVtxbeforehist = new TH1F("nVtxbeforehist","Number of Good Vertex Before",100,0,500);
        TH1F *pfMethist = new TH1F("pfMethist","pfMet Distribution",100,0,500);
        TH1F *pfMetPhihist = new TH1F("pfMetPhihist","pfMetPhi Distribution",10,-5,5);
        TH1F *Elehist = new TH1F("Elehist","Number of Electrons",10,0,10);
        TH1F *Muhist = new TH1F("Muhist","Number of Muons",10,0,10);
        TH1F *Tauhist = new TH1F("Tauhist","Number of Taus",10,0,10);
        TH1F *nGoodTHINJetshist = new TH1F("nGoodTHINJets", "Number of Good Thin Jets", 100, 0, 100);
        TH1F *nGoodTHINBJetshist = new TH1F("nGoodTHINBJets", "Number of Good b Jets", 100, 0, 100);
      ...
      for(Long64_t jEntry=0; jEntry<data.GetEntriesFast() ;jEntry++){
        ...
        float pfMetPhi = data.GetFloat("pfMetCorrPhi");
          ...
          pfMethist->Fill(pfMet);
          pfMetPhihist->Fill(pfMetPhi);
        ...
        for(int ij=0; ij < nTHINJets; ij++){
          ...
          nGoodTHINJetshist->Fill(nGoodTHINJets);
        if(nGoodTHINJets>1)continue;
        nPass[8]++;
          
          nGoodTHINBJetshist->Fill(nGoodTHINBJets);
      ...
      efferr(nPass[10],nTotal);
        ...
        string pdfName = "coba1.pdf";
        TCanvas *c1 = new TCanvas("c1", "c1", 800, 600);
        //c1->Divide(1, 5);
        c1->Print((pdfName+"[").data());
        nVtxbeforehist->Draw("hist");
        c1->Print(pdfName.data());
        pfMethist->Draw("hist");
        c1->Print(pdfName.data());
        pfMetPhihist->Draw("hist");
        c1->Print(pdfName.data());
        Elehist->Draw("hist");
        c1->Print(pdfName.data());
        Muhist->Draw("hist");
        ...
        Tauhist->Draw("hist");
        c1->Print(pdfName.data());
        nGoodTHINJetshist->Draw("hist");
        c1->Print(pdfName.data());
        nGoodTHINBJetshist->Draw("hist");
        c1->Print(pdfName.data());
        c1->Print((pdfName+"]").data());
